...
async def _request_script(digest_block: str, mood: Mood, settings: Settings) -> str:
    ...
    if not settings.openrouter_api_key:
        raise VoiceError("OPENROUTER_API_KEY не задан")
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.llm.timeout_sec) as client:
            response = await client.post(_OPENROUTER_URL, json=body, headers=headers)
    except httpx.HTTPError as exc:
        raise VoiceError(f"сетевая ошибка OpenRouter: {exc}") from exc
    if response.status_code != 200:
        raise VoiceError(f"OpenRouter ответил {response.status_code}")
    try:
        return response.json()["choices"][0]["message"]["content"]
    except (KeyError, IndexError, TypeError, ValueError) as exc:
        raise VoiceError("неожиданный формат ответа OpenRouter") from exc
