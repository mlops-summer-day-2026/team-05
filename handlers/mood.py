...
async def _run_digest(
    ...
    async def progress(text: str) -> None:
        ...
        if text != progress_handle["last"]:
            await bot.edit_message_text(
                text, chat_id=chat_id, message_id=status.message_id
            )
    ...
    try:
        ...
        await bot.edit_message_text(
            settings.get_text("no_channels"), chat_id=chat_id, message_id=status.message_id
        )
        return
    except NoPostsError:
        ...
        await bot.edit_message_text(
            settings.get_text("error", error=str(exc)), chat_id=chat_id, message_id=status.message_id
        )
        return
...
async def _resolve_result(
    ...
    try:
        ...
        await bot.edit_message_text(
            settings.get_text("error", error="таймаут пайплайна"),
            chat_id=chat_id,
            message_id=status.message_id,
        )
        return None
        ...
        await bot.edit_message_text(
            settings.get_text("error", error=str(exc)),
            chat_id=chat_id,
            message_id=status.message_id,
        )
        return None
...
async def on_voice(callback: CallbackQuery, bot: Bot, settings: Settings, db: Database) -> None:
    ...
    try:
        ...
        await bot.edit_message_text(
            settings.get_text("voice_error", error=str(exc)),
            chat_id=chat_id,
            message_id=status.message_id,
        )
        return
